...
#define BUFFER_OFFSET(i) ((char *)NULL + (i))
    ...
    // Model Matrix
    btTransform rb;                         // physics rigidbody. contains the position
    gameObject.rigidbody->getMotionState()->getWorldTransform(rb);
    Transform* t = gameObject.transform;    // transform of the object. used for the OFFSET of the model.
    
    // position in world
    GLKMatrix4 m = GLKMatrix4Translate(GLKMatrix4Identity, rb.getOrigin().getX(), rb.getOrigin().getY(), rb.getOrigin().getZ());
    // model position offset
    m = GLKMatrix4Translate(m, t.position.x, t.position.y, t.position.z);
    
    // rotation in world
    GLKQuaternion rotation = GLKQuaternionMake(rb.getRotation().getX(), rb.getRotation().getY(), rb.getRotation().getZ(), rb.getRotation().getW());
    GLKMatrix4 mRot = GLKMatrix4MakeWithQuaternion(rotation);
    m = GLKMatrix4Multiply(m, mRot);
    // model rotation offset
    m = GLKMatrix4Rotate(m, GLKMathDegreesToRadians(t.rotation.y), 0.0, 1.0, 0.0 );
    m = GLKMatrix4Rotate(m, GLKMathDegreesToRadians(t.rotation.x), 1.0, 0.0, 0.0 );
    m = GLKMatrix4Rotate(m, GLKMathDegreesToRadians(t.rotation.z), 0.0, 0.0, 1.0 );
    
    // model scale offset
    m = GLKMatrix4Scale(m, t.scale.x, t.scale.y, t.scale.z);
